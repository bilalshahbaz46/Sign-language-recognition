...
public class MainActivity extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2,View.OnTouchListener {
    ...
    static private double[][] values ={{165, 286, 198, 184, 146},//A
            {165, 287, 200, 186, 155},
            {165, 286, 200, 186, 156},
            {163, 288, 200, 187, 163},
            {173, 288, 201, 188},
    ...
    private Mat RGBA;//This is the main Mat for capturing frame
    private Mat RGBAT;//This mat is used for taking transpose of the frames captured in previous Mat
    private Mat ContMat;//It is used to store contour
    private Mat TouchedRGB;//This mat is used to calculate the area of Interest
    private Mat TouchedHSV;//This will be used to convert RGB ROI into HSV
    private Mat HSV;
    private Rect Rectangle;
    private Scalar hsvColor;// This is used to sum ROI for average
    private Scalar RGBScalar;// To average color of ROI
    private Boolean colorselect = false;
    private Scalar LowerBound;//This is used to keep lower bound of ROI
    private Scalar UpperBound;//This used to keep upper bound of ROI
    private Mat HSVMatApplied;// A Mat used to fill all the calculated results
    private Mat Hierarchy;// used in contours
    private Scalar ColorRadius;
    private Point Centerofgravity = new Point();// center point of the contour
    private int contourAxisAngle;
    private int division = 5;
    private ArrayList<Point> fingerpoints = new ArrayList<Point>();
    private static final int Maxnumberofpoint = 40;
    private static final int Max_finger_distance = 20;
    private static String[] AlphaVal = new String[values.length];
    private double[] valueOfFingers, valueBetweenFingers;
    ...
    protected void onResume() {
        ...
        if (OpenCVLoader.initDebug()) {
            ...
            Log.d("TAG", "not loaded successfully");
    ...
    public void onCameraViewStarted(int width, int height) {
        ...
        HSVMatApplied = new Mat(height, width, CvType.CV_8UC3);
        ContMat = new Mat(HSVMatApplied.height(), HSVMatApplied.width(), CvType.CV_8UC1);
        TouchedHSV = new Mat();
        TouchedRGB = new Mat();
        Rectangle = new Rect();
        ...
        Hierarchy = new Mat();
        ColorRadius = new Scalar(25, 50, 50, 0);
    ...
    public Mat onCameraFrame(CameraBridgeViewBase.CvCameraViewFrame inputFrame) {
        ...
        Imgproc.blur(RGBA, RGBA, new Size(7, 7));
        Imgproc.cvtColor(RGBAT, HSV, Imgproc.COLOR_RGB2HSV);
        if (colorselect) {
            Mat colorlabel = RGBAT.submat(4, 68, 4, 68);
            colorlabel.setTo(RGBScalar);
        }
        Core.inRange(HSV, LowerBound, UpperBound, HSVMatApplied);
        //Morphological operations
        Imgproc.blur(HSVMatApplied, HSVMatApplied, new Size(1, 1));
        Mat dilation = Imgproc.getStructuringElement(Imgproc.MORPH_RECT, new Size(1, 1));
        Mat Erosion = Imgproc.getStructuringElement(Imgproc.MORPH_RECT, new Size(1, 1));
        Imgproc.erode(HSVMatApplied, HSVMatApplied, Erosion);
        Imgproc.dilate(HSVMatApplied, HSVMatApplied, dilation);
        Imgproc.blur(HSVMatApplied, HSVMatApplied, new Size(1, 1));
        // Mat for the contour
        Mat ContMat = new Mat(HSVMatApplied.height(), HSVMatApplied.width(), CvType.CV_8UC1);
        //Finding Contours
        List<MatOfPoint> contours = new ArrayList<MatOfPoint>();
        Imgproc.findContours(HSVMatApplied, contours, Hierarchy, Imgproc.RETR_EXTERNAL, Imgproc.CHAIN_APPROX_SIMPLE);
        ...
        for (int contourID = 0; contourID < contours.size(); contourID++) {
            ...
            if (MaxValue < ContourArea) {
                ...
                MaxValueID = contourID;
            }

        }
        Imgproc.drawContours(RGBAT, contours, MaxValueID, new Scalar(255, 0, 0, 255), 1);
        if (MaxValueID > 0) {
            MatOfPoint bigContour = contours.get(MaxValueID);
            find_moment(bigContour, 1);
            find_fingers(bigContour, 1);
            ...
            valueOfFingers = new double[fingerpoints.size()];
            valueBetweenFingers = new double[fingerpoints.size()];

            lines = RGBAT;


        }
        if (RGBAT.height() > 0 && RGBAT.width() > 0 && fingerpoints.size() != 0) {
            for (int i = 0; i < fingerpoints.size(); i++) {

                Point center = new Point(RGBAT.width() / 2, RGBAT.height() / 2);
                Point topLeft = new Point(RGBAT.width() / division, 0);
                Point topRight = new Point(RGBAT.width() / division, RGBAT.height());
                Point bottomLeft = new Point((division - 1) * RGBAT.width() / division, 0);
                Point bottomRight = new Point((division - 1) * RGBAT.width() / division, RGBAT.height());
                Imgproc.circle(lines, center, 5, new Scalar(0, 0, 0));
                Imgproc.line(lines, topLeft, topRight, new Scalar(0, 0, 0), 2);
                Imgproc.line(lines, bottomLeft, bottomRight, new Scalar(0, 0, 0), 2);

                Point pt1 = fingerpoints.get(i);
                Imgproc.line(lines, Centerofgravity, pt1, new Scalar(148,75,90), 2);
                Imgproc.circle(lines, pt1, 16, new Scalar(225, 156, 0));
                if (i > 0) {
                    Point pt2 = fingerpoints.get(i - 1);
                ...
                valueOfFingers[i] = (Math.sqrt(Math.pow((pt1.x - Centerofgravity.x), 2) + Math.pow((pt1.y - Centerofgravity.y), 2)));
                System.out.println(valueOfFingers[i]);

            }

            Imgproc.circle(lines, Centerofgravity, 90, new Scalar(0, 0, 255));

            runOnUiThread(new Runnable() {
                public void run() {
                    for (int i = 0; i < values.length; i++) {
                        boolean Matched = true;
                        if (values[i].length==valueOfFingers.length) {
                            for (int j = 0; j < valueOfFingers.length; j++) {
                                if (valueOfFingers[j] <= (values[i][j] + 20) && valueOfFingers[j] >= (values[i][j] - 20)) {
                                    Matched &= true;
                                } else {
                                    Matched &= false;
                                }
                            ...
                            if (Matched == true && (new String(AlphaVal[i]).equals("A"))){
                            ...
                            else if (Matched == true && (new String(AlphaVal[i]).equals("D"))){
                            ...
                            else if (Matched == true && (new String(AlphaVal[i]).equals("E"))){
                            ...
                            else if (Matched == true && (new String(AlphaVal[i]).equals("I"))){
                            ...
                            else if (Matched == true && (new String(AlphaVal[i]).equals("M"))){
                            ...
                            else if (Matched == true && (new String(AlphaVal[i]).equals("R"))){
                            ...
                            else if (Matched == true && (new String(AlphaVal[i]).equals("Y"))){
                            ...
                            if (Matched == true && !(new String(gesture).equals(previousGesture))) {
                                Toast.makeText(MainActivity.this, AlphaVal[i], Toast.LENGTH_SHORT).show();
    ...
    private void find_moment(MatOfPoint bigContour, int scale) {
        Moments moments = Imgproc.moments(bigContour);
        List<Moments> My_Moments = new ArrayList<Moments>();
        My_Moments.add(moments);
        List<Point> centerpoint=new ArrayList<Point>(My_Moments.size());
        //Finding center point and publishing it on the contour
        for (int i = 0; i < My_Moments.size(); i++) {
            Point Cp=new Point(My_Moments.get(i).m10 / (My_Moments.get(i).m00 + 1e-5), My_Moments.get(i).m01 / (My_Moments.get(i).m00 + 1e-5));
            centerpoint.add(Cp);
            Imgproc.circle(RGBAT, centerpoint.get(i),6, new Scalar(255,55,10), -1);
        }
        ...
        if (m00 != 0) { // calculate center
            ...
            Centerofgravity.x = xCenter;
            Centerofgravity.y = yCenter;
        }
        ...
        if (fingerpoints.size() > 0) {
            int yTotal = 0;
            for (Point pt : fingerpoints)
                yTotal += pt.y;
            int avgYFinger = yTotal / fingerpoints.size();
            if (avgYFinger > Centerofgravity.y) // fingers below COG
    ...
    private int calculateTilt(double m11, double m20, double m02) {
        ...
        double theta = 0.5 * Math.atan2(2 * m11, difference);
        int tilt = (int) Math.round(Math.toDegrees(theta));

        if ((difference > 0) && (m11 == 0))
            return 0;
        else if ((difference < 0) && (m11 == 0))
            return -90;
        else if ((difference > 0) && (m11 > 0)) // 0 to 45 degrees
            return tilt;
        else if ((difference > 0) && (m11 < 0)) // -45 to 0
            return (180 + tilt); // change to counter-clockwise angle
        else if ((difference < 0) && (m11 > 0)) // 45 to 90
            return tilt;
        else if ((difference < 0) && (m11 < 0)) // -90 to -45
            return (180 + tilt); // change to counter-clockwise angle
    ...
    private void find_fingers(MatOfPoint bigContour, int scale) {
        ...
        Point farPoint=null;
        float depth=0;
        ArrayList<Point> Farpoint=new ArrayList<>();
        ArrayList<Point> StartPoint =new ArrayList<>();
        ArrayList<Point> EndPoint=new ArrayList<>();
        ArrayList<Integer> Depth=new ArrayList<>();
        Imgproc.convexHull(bigContour,hull,false);
        Imgproc.convexityDefects(bigContour,hull,defects);
        int totaldefects= (int) defects.total();
        if(totaldefects>Maxnumberofpoint){
            totaldefects=Maxnumberofpoint;
        }
        for(int B=0;B<defects.toList().size();B=B+4){
            startPoint= bigContour.toList().get(defects.toList().get(B));
            endPoint= bigContour.toList().get(defects.toList().get(B+1));
            farPoint = bigContour.toList().get(defects.toList().get(B+2));
            depth = defects.toList().get(B+3);
            StartPoint.add(new Point((int)Math.round(startPoint.x*scale),(int)Math.round(startPoint.y*scale)));
            EndPoint.add(new Point((int)Math.round(endPoint.x*scale),(int)Math.round(endPoint.y*scale)));
            Farpoint.add(new Point((int)Math.round(farPoint.x*scale),(int)Math.round(farPoint.y*scale)));
            Depth.add((int) (depth*1));

        }
        reduceTips(totaldefects, StartPoint);
    }

    private void reduceTips(int totaldefects, ArrayList<Point> startPoint) {
        fingerpoints.clear();

        for(int i=0;i<totaldefects;i++){
            if(i>0){
                if((Math.abs(startPoint.get(i).x-startPoint.get(i-1).x))<Max_finger_distance && (Math.abs(startPoint.get(i).y - startPoint.get(i-1).y)<Max_finger_distance)){
                    continue;
                }
            }
            if (i == (totaldefects - 1)) {
                if ((Math.abs(startPoint.get(totaldefects-1).x - startPoint.get(0).x)) < Max_finger_distance
                        && (Math.abs(startPoint.get(totaldefects-1).y-startPoint.get(0).y) < Max_finger_distance)) {
                    continue;
                }
            }
            if(startPoint.get(i).x<RGBAT.width()/division){
                continue;
            }
            if (startPoint.get(i).x > (division - 1)*RGBAT.width()/division) {
                continue;
            }
            if (angle(startPoint.get(i)) < 90) {
                continue;
            }
            fingerpoints.add(startPoint.get(i));
        }
    }
    // Finding the angle between two consecutive tips and the fold between those tips
    private int angle(Point tip){
        return Math.abs((int) Math.round(Math.toDegrees(Math.atan2(tip.x, tip.y - ((2*RGBAT.height())/3)))));
    }

    @Override
    public boolean onTouch(View v, MotionEvent event) {
        int Columns = RGBAT.cols();
        int Rows = RGBAT.rows();
        int Xoffset = (javaCameraView.getWidth() - Columns) / 2;
        ...
        if (X + 4 < Columns) {
            Rectangle.width = X + 4 - Rectangle.x;
        } else {
            Rectangle.width = Columns - Rectangle.x;
        }
        if (Y + 4 < Rows) {
            Rectangle.height = Y + 4 - Rectangle.y;
        } else {
            Rectangle.height = Rows - Rectangle.y;
        }
        TouchedRGB = RGBAT.submat(Rectangle);
        // System.out.println(TouchedRGB);
        Imgproc.cvtColor(TouchedRGB, TouchedHSV, Imgproc.COLOR_RGB2HSV);
        //System.out.println(TouchedHSV);
        hsvColor = Core.sumElems(TouchedHSV);
        // System.out.println(hsvColor);
        int Points = Rectangle.width * Rectangle.height;
        for (int i = 0; i < hsvColor.val.length; i++) {
            hsvColor.val[i] /= Points;
            //  System.out.println(hsvColor.val[i]);
        }
        RGBScalar = convertHSV2RGB(hsvColor);
